...
def create_item_scan_log():
    try:
        ...
        tag_number = data.get("tag_number")
           # to Always override status if validations pass 
        # status = "Counted"
        ...
        # Validate tag is linked with production item
        mapping_valid = frappe.db.exists(
            "Production Item Tag Map",
            {"production_item": production_item, "tracking_tag": tag_number, "is_active": 1}
        )
        if not mapping_valid:
            return {"status": "error", "message": _("Tag {0} not linked to Production Item {1}").format(tag_number, production_item)}
